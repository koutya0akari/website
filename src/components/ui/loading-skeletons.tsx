  ...
  return (
    <div className={containerClass} aria-hidden>
      <div className="animate-pulse space-y-4">
        <div className="h-3 w-32 rounded bg-night-muted" />
        <div className="h-9 w-64 rounded-lg bg-night-muted" />
      </div>
      <div className="grid animate-pulse gap-6 sm:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: cards }).map((_, i) => (
          <div key={i} className="h-48 rounded-2xl border border-transparent bg-night-muted" />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-content px-4 py-8 sm:px-6 sm:py-12" aria-hidden>
      <div className="animate-pulse space-y-6">
        <div className="h-3 w-28 rounded bg-night-muted" />
        <div className="space-y-3">
          <div className="h-10 w-3/4 rounded-lg bg-night-muted" />
          <div className="h-4 w-40 rounded bg-night-muted" />
        </div>
        <div className="h-56 rounded-2xl bg-night-muted" />
        <div className="space-y-3">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="h-4 rounded bg-night-muted"
