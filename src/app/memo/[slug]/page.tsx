...
import { getMemoBySlug } from "@/lib/memo";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const entry = await getMemoBySlug(slug);
  if (!entry) {
    return { title: MEMO_LABEL };
  }
...
export default async function MemoDetailPage({ params }: PageProps) {
  ...
  const summary = createExcerpt(stripHtml(entry.summary || entry.body), 260);
  const shareUrl = `${SITE_URL}/memo/${entry.slug}`;
