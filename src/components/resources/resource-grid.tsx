...
export function ResourceGrid({ resources }: ResourceGridProps) {
  ...
  return (
    <div className="grid gap-4">
      {resources.map((item) => (
        <article key={item.id} className="flex flex-col gap-3 rounded-2xl border border-transparent bg-night-muted p-6 sm:flex-row sm:items-center sm:justify-between">
